...
class SelectLayerUpper(nn.Module):
    def __init__(self):
        ...
        # 3*9 : 3*9 + 3 
        # 3*13: 3*13 +3   
        self.EE_upper_idx = [ 0,1,2, (9)*3+0,(9)*3+1,(9)*3+2, (10)*3+0,(10)*3+1,(10)*3+2, (11)*3+0,(11)*3+1,(11)*3+2, (12)*3+0,(12)*3+1,(12)*3+2, 
        (13)*3+0,(13)*3+1,(13)*3+2, (14)*3+0,(14)*3+1,(14)*3+2, (15)*3+0,(15)*3+1,(15)*3+2, (16)*3+0,(16)*3+1,(16)*3+2,
        (17)*3+0,(17)*3+1,(17)*3+2, (18)*3+0,(18)*3+1,(18)*3+2, (19)*3+0,(19)*3+1,(19)*3+2, (20)*3+0,(20)*3+1,(20)*3+2]


    def forward(self, input,reverse=False):
        if not reverse:
            self.mask = torch.zeros(input.shape).to(input.device)
            self.mask[:,self.EE_upper_idx,:] = 1
            
            output = input[:,self.EE_upper_idx,:]
...
class SelectLayerLower(nn.Module):
    def __init__(self):
        ...
        # 3*21,,23
        self.EE_lower_idx = [ 
        (5)*3+0,(5)*3+1,(5)*3+2, (6)*3+0,(6)*3+1,(6)*3+2, (7)*3+0,(7)*3+1,(7)*3+2, (8)*3+0,(8)*3+1,(8)*3+2,
        (1)*3+0,(1)*3+1,(1)*3+2, (2)*3+0,(2)*3+1,(2)*3+2, (3)*3+0,(3)*3+1,(3)*3+2, (4)*3+0,(4)*3+1,(4)*3+2]
        
    def forward(self, input,reverse=False):
        if not reverse:
            self.mask = torch.zeros(input.shape).to(input.device)
            self.mask[:,self.EE_lower_idx,:] = 1

            output = input[:,self.EE_lower_idx,:]
            self.masked_pose = input.mul(self.mask)
...
class FlowNet(nn.Module):
    ...
    def forward(self, z, cond, logdet=0., reverse=False, eps_std=None):
        if not reverse:
            ...
            # log likelihood for upper body
            logdet = self.distribution.logp(z_u) + logdet
            ...
            # log likelihood for lower body 
            logdet = self.distribution.logp(z_l) + logdet

            # concate z space & logdet
            z = torch.cat((z_u,z_l),dim=1)
            return z, logdet
        else:
            # train z->x
            # select upper body first, (fixed hierarcy)
